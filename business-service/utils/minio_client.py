...
class MinIOClient:
    ...
    def get_presigned_url(
        ...
        Returns:
            预签名 URL，如果对象不存在返回 None
        """
        try:
            # 先检查对象是否存在
            if not self.file_exists(bucket_name, object_name):
                print(f"⚠️ 对象不存在: {bucket_name}/{object_name}", flush=True)
                return None
            ...
            print(f"❌ 生成URL失败: {e}", flush=True)
            return None
        except Exception as e:
            print(f"❌ 生成URL异常: {e}", flush=True)
            import traceback
            traceback.print_exc()
            return None
    ...
    def get_file_stream(self, bucket_name: str, object_name: str):
        """
        直接从 MinIO 获取文件流和元信息，用于后端代理返回给浏览器。
        返回 (response, content_type, content_length)，失败返回 None
        """
        try:
